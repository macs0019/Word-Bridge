...
const useWindowSize = (centerContainerRef) => {
    ...
    useEffect(() => {
        const handleResize = () => {
            ...
            const newHeightPx = newHeight - alturaVhEnPixeles + "px";
            if (deviceWidth <= 1800 && centerContainerRef.current) {
                ...
                const altura90vhEnPixeles = (90 * alturaViewport) / 100;

                // Restar 32px (asegúrate de convertir la altura a un número antes de restar)
                const nuevaAlturaEnPixeles = altura90vhEnPixeles - alturaVhEnPixeles;
